...
def remove_file_to_remote(file_path: str, account: sftp_account) -> bool:
    """
    Function to remove a file to a remote server using SFTP.
    # file_path must be a valid file path with an extension.
    """
    if "." not in file_path:
        print("Invalid file path. Please provide a valid file path with an extension.")
        return False

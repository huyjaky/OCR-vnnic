...
13. `ThoiDiemDangKy`: Thời điểm đăng ký (ISO 8601)
14. `SoHopDong`: Số hợp đồng (string)
15. `NgayCoHieuLucHopDong`: Ngày hiệu lực hợp đồng (YYYY-MM-DD)
16. `GiaTriKhoanVay`: Giá trị khoản vay (number hoặc null)
17. `SoPhuLuc`: Số phụ lục (string hoặc null)
18. `ThoiDiemKHDangKy`: Thời điểm KH đăng ký (ISO 8601 hoặc null)
19. `ThoiDiemDKLanDau`: Thời điểm đăng ký lần đầu (ISO 8601)
20. `NoiDungThayDoi`: Nội dung thay đổi (string hoặc null)
21. `MoTaChungTaiSan`: Mô tả chung tài sản (string)
22. `BenGiao`: Mảng xuất hiện các đối tượng thuộc loại `CHỦ THỂ` (`BenNhan`) (ít nhất 1 item)
23. `BenNhan`: Mảng xuất hiện các đối tượng thuộc loại `CHỦ THỂ` (`BenGiao`) (ít nhất 1 item)
24. `TaiSan`: Mảng xuất hiện các đối tượng thuộc loại `TÀI SẢN` (ít nhất 1 item)
